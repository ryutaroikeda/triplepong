...
class GameEvent(Event):
    ...
    def Serialize(self):
        ...
        for k in self.keys:
            flag += k
        return struct.pack(GameEvent.FORMAT, self.event_type, self.flag,
                self.frame)
    ...
    def Deserialize(self, b):
        ...
        (flag, self.frame,) = struct.unpack(GameEvent.SUBFORMAT, b)
        if flag & GameEvent.EVENT_FLAP_LEFT_PADDLE:
            self.keys.append(GameEvent.EVENT_FLAP_LEFT_PADDLE)
        if flag & GameEvent.EVENT_FLAP_RIGHT_PADDLE:
            self.keys.append(GameEvent.EVENT_FLAP_RIGHT_PADDLE)
        if flag & GameEvent.EVENT_FLAP_BALL:
            self.keys.append(GameEvent.EVENT_FLAP_BALL)
        pass
    pass
